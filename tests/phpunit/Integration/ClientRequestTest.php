...
class ClientRequestTest extends \WP_UnitTestCase {
	public function setUp() {
		...
		$this->client = $this->getMockBuilder( '\AudioTheme_Agent_Client' )
			->setMethods( array( 'disconnect', 'refresh_access_token', 'wp_remote_request' ) )
			->getMock();
	...
	public function test_unauthenticated_request() {
		$client = $this->getMockBuilder( '\AudioTheme_Agent_Client' )
			->setMethods( array( 'is_authorized', 'disconnect', 'refresh_access_token', 'wp_remote_request' ) )
			->getMock();

		$client->method( 'is_authorized' )->willReturn( false );

		$client->expects( $this->exactly( 0 ) )->method( 'disconnect' );
		...
		$args = $client->request( 'https://example.com/api/v1/public_resources' );

		$this->assertArrayNotHasKey( 'Authorization', $args['headers'] );
	}

	public function test_authenticated_request() {
		$this->client->expects( $this->exactly( 0 ) )->method( 'disconnect' );
		...
		$args = $this->client->request( 'https://example.com/api/v1/protected_resources' );
	...
	public function test_authenticated_request_with_401_response_code() {
		$this->client = $this->getMockBuilder( '\AudioTheme_Agent_Client' )
			->setMethods( array( 'disconnect', 'refresh_access_token', 'wp_remote_request', 'wp_remote_retrieve_response_code' ) )
			->getMock();
		...
		$response = $this->client->request( 'https://example.com/api/v1/protected_resources' );
	...
	public function test_authenticated_request_with_invalid_token() {
		...
		$this->client->expects( $this->exactly( 1 ) )->method( 'disconnect' );
		...
		$this->client->request( 'https://example.com/api/v1/protected_resources' );
	}
